              ...
              <div className="card-body">
                ...
                <div className="field-grid">
                  <FieldLabelWithInfo
                    ...
                    hint={metaAwsRegion?.description || "AWS GovCloud region where the OpenShift cluster will be deployed. IMPORTANT: AWS GovCloud is a SEPARATE cloud partition from AWS Commercial - it has physically and logically isolated infrastructure, different API endpoints, separate account system, and stricter compliance controls (FedRAMP High, DoD IL2-5, ITAR, CJIS). You MUST have an AWS GovCloud account to use these regions - commercial AWS accounts cannot access GovCloud regions. WHY IT MATTERS: GovCloud regions are designed for US federal, state, local, and tribal government customers, plus contractors handling controlled unclassified information (CUI) or export-controlled data. All GovCloud datacenters are on US soil, staffed by US citizens, with enhanced audit/compliance capabilities. Common GovCloud regions: 'us-gov-west-1' (US West - Oregon/California), 'us-gov-east-1' (US East - Ohio). CRITICAL: Once you choose a region, the cluster is permanently tied to it - you cannot migrate to a different region without rebuilding. Region choice impacts latency to end users, disaster recovery planning (each region is independent with separate AZs), and compliance (some regulations mandate specific geographic locations). For multi-AZ high availability, the installer automatically spreads nodes across availability zones within the selected region (most GovCloud regions have 3 AZs). NETWORK CONNECTIVITY: Ensure your network can reach the selected region's API endpoints and that your mirror registry (if disconnected) is accessible from that region. Example: 'us-gov-west-1' for West Coast federal agencies, 'us-gov-east-1' for East Coast deployments."}
                ...
                <div className="field-grid">
                  <FieldLabelWithInfo
                    label="VPC mode"
                    hint="Choose whether the OpenShift installer creates a new VPC and subnets (installer-managed, the default) or you provide existing subnet IDs (existing subnets, common for production/regulated environments). WHAT IS A VPC: A Virtual Private Cloud (VPC) is an isolated virtual network in AWS where you launch resources (EC2 instances, load balancers, databases). It has its own IP address range (CIDR), subnets across availability zones, route tables, internet gateways, and security groups. INSTALLER-MANAGED VPC (default): The installer creates a new VPC, subnets (public and private subnets across multiple AZs), internet gateway, NAT gateways, route tables, and security groups automatically. This is the simplest option - no pre-configuration needed, good for development/testing or when you have no existing network infrastructure. The installer cleans up the VPC if installation fails. EXISTING SUBNETS: You provide pre-created subnet IDs, and the installer uses them for cluster nodes. Choose this when: (1) Your organization has centralized network teams managing VPCs (common in enterprises). (2) Compliance requires specific network topologies (e.g., no internet gateways, private-only subnets, custom routing). (3) You need to integrate with existing on-premises networks via VPN/Direct Connect. (4) You want to share a VPC across multiple clusters or applications. (5) Disconnected/airgap installs (no public subnets or internet access). IMPORTANT: When using existing subnets, you must provide subnet IDs in the 'Subnets' field below - typically 3+ subnet IDs spread across different availability zones for high availability (one subnet per AZ). The subnets must have sufficient IP addresses available (at least 20-30 IPs per subnet for a small cluster), correct routing (public subnets need internet gateway route, private subnets need NAT gateway or proxy for outbound), and proper security group rules (allow cluster traffic). NETWORKING TAB VS VPC SUBNETS: The Networking tab's Machine/Cluster/Service CIDRs are for internal Kubernetes networking (pod IPs, service IPs) - they are separate from the AWS VPC subnet CIDRs (where EC2 instances get IPs). Example: Choose 'Installer-managed' for quick dev clusters, 'Existing subnets' for production with pre-configured networking."}
                  >
                    <select
                      value={awsVpcMode}
                          ...
                          return (
                            <div key={idx} className="list-item platform-specifics-aws-subnet-row">
                              ...
                              <FieldLabelWithInfo
                                ...
                                hint="Subnet roles define which OpenShift infrastructure components can use each subnet in existing VPC deployments (OpenShift 4.20+). Leave unassigned to let the installer auto-assign roles, or explicitly assign roles for fine-grained control. WHAT ARE ROLES: When you provide existing subnets, OpenShift needs to know which subnets to use for which purposes (cluster nodes, bootstrap, load balancers). Roles tag subnets with their intended use. AVAILABLE ROLES: 'ClusterNode' (control plane and worker node VMs land here - required), 'BootstrapNode' (temporary bootstrap VM - required for install), 'IngressControllerLB' (load balancer for ingress/apps traffic - *.apps.<cluster>.<domain>), 'ControlPlaneExternalLB' (load balancer for external API access - api.<cluster>.<domain> - skip this if publish=Internal), 'ControlPlaneInternalLB' (load balancer for internal API access). HOW TO ASSIGN: Multi-select one or more roles per subnet. For example, a public subnet might have roles: ClusterNode, BootstrapNode, IngressControllerLB, ControlPlaneExternalLB. A private subnet might have: ClusterNode, ControlPlaneInternalLB. IMPORTANT RULES: (1) If you assign ANY role to ANY subnet, you MUST assign at least one role to EVERY subnet (no mix of auto-assigned and manually-assigned). (2) All required roles must be covered across your subnets - you cannot omit ClusterNode or BootstrapNode. (3) For External publish mode, include ControlPlaneExternalLB on at least one subnet. (4) Subnets can have multiple roles - a single subnet can be used for nodes AND load balancers. WHEN TO LEAVE BLANK: Leave roles unassigned for simpler deployments - the installer will auto-assign roles based on subnet types (public vs private) and tags. WHEN TO SET EXPLICITLY: Set roles when you have specific subnet isolation requirements (e.g., load balancers only in public subnets, nodes only in private subnets), or when the auto-assignment doesn't match your network architecture. Example: Public subnet gets ClusterNode + BootstrapNode + IngressControllerLB + ControlPlaneExternalLB, Private subnet gets ClusterNode + ControlPlaneInternalLB."}
                              >
                                <select
                                  multiple
                                  value={rolesVal}
                ...
                <div className="field-grid">
                  <FieldLabelWithInfo
                    ...
                    hint={metaAwsHostedZone?.description || "AWS Route 53 hosted zone ID for your base domain (the parent domain under which the OpenShift cluster DNS records will be created). Leave blank UNLESS you are using existing subnets (VPC mode = Existing subnets) AND you have a pre-existing Route 53 hosted zone for your base domain. WHAT IS ROUTE 53 HOSTED ZONE: Route 53 is AWS's managed DNS service. A hosted zone is a container for DNS records for a specific domain (e.g., 'example.com'). It contains records like A, CNAME, NS that route traffic to your infrastructure. When you create a hosted zone, AWS assigns it a unique ID like 'Z1234567890ABC'. WHY YOU MIGHT SET THIS: When using existing VPC/subnets, you often have an existing Route 53 hosted zone for your corporate domain. The OpenShift installer needs to create DNS records for the cluster (api.<cluster>.<domain>, *.apps.<cluster>.<domain>, etcd records, etc.). By providing the hosted zone ID, you tell the installer where to create those records. WHEN TO LEAVE BLANK: (1) Installer-managed VPC mode - the installer creates a new hosted zone automatically. (2) You're not using Route 53 for DNS (e.g., using external DNS servers, bind, corporate DNS). (3) You'll manage DNS records manually or via automation outside the installer. IMPORTANT: The hosted zone must exist BEFORE installation and must be for your base domain (set in Identity & Access step). For example, if base domain is 'openshift.example.com', you need a hosted zone for 'openshift.example.com' (or 'example.com' if you're creating a subdomain). The zone must be in the same AWS account (unless using shared VPC with cross-account access, see 'Hosted zone role ARN' below). FINDING THE HOSTED ZONE ID: In AWS Console → Route 53 → Hosted zones → click your domain → copy the 'Hosted zone ID' (starts with Z, ~13-14 characters). Or via AWS CLI: 'aws route53 list-hosted-zones-by-name --dns-name example.com'. Example: 'Z1234567890ABC' (but use your actual hosted zone ID). Most installer-managed VPC installs should leave this blank."}
                  >
                    <input
                  ...
                  {platformConfig.aws?.hostedZoneSharedVpc ? (
                    <FieldLabelWithInfo
                      ...
                      hint="Amazon Resource Name (ARN) of an IAM role in the AWS account that owns the Route 53 hosted zone, used for cross-account DNS record creation in shared VPC scenarios. REQUIRED ONLY when 'Hosted zone is in another account (shared VPC)' checkbox is enabled. WHAT IS THIS: In enterprise AWS environments, you often have separate AWS accounts - e.g., a central 'networking' account owns VPCs and DNS (Route 53 hosted zones), while application teams have separate accounts for running workloads. When you install OpenShift in account A but the Route 53 hosted zone lives in account B, the installer needs permission to create DNS records in account B's hosted zone. You grant this permission via a cross-account IAM role. HOW IT WORKS: (1) In the account that owns the Route 53 hosted zone (account B), create an IAM role with Route 53 permissions (route53:ChangeResourceRecordSets, route53:ListHostedZones, route53:GetChange on the hosted zone). (2) Configure the role's trust policy to allow the installation account (account A) to assume the role. (3) Copy the role's ARN and enter it here. (4) The installer uses STS AssumeRole to temporarily assume this role when creating DNS records. ARN FORMAT: 'arn:aws-us-gov:iam::<account-id>:role/<role-name>' for AWS GovCloud (note 'aws-us-gov' partition, not 'aws'). Example: 'arn:aws-us-gov:iam::123456789012:role/OpenShiftDNSAccess'. IMPORTANT: The role must have permissions on the specific hosted zone you specified in 'Hosted zone ID' above. The trust policy must allow the AWS account where you're installing OpenShift to assume the role (typically via the IAM user or role credentials you use for installation). WHEN TO USE: Only for shared VPC + cross-account Route 53 scenarios. If your hosted zone and cluster are in the same AWS account, DO NOT set this - leave the checkbox unchecked. Common in large enterprises with centralized networking teams. You can verify the role ARN in AWS Console → IAM → Roles → <role-name> → ARN, or via CLI: 'aws iam get-role --role-name OpenShiftDNSAccess'. Example: 'arn:aws-us-gov:iam::999888777666:role/SharedVPC-Route53-Role'."}
                    >
                      <input
                ...
                <div className="field-grid">
                  <FieldLabelWithInfo
                    ...
                    hint="AWS load balancer type to use for cluster API and default ingress router endpoints. Leave blank to use AWS/OpenShift defaults (typically Classic for older regions, NLB for newer). WHAT ARE THESE: AWS offers multiple load balancer types - Classic ELB (Elastic Load Balancer, the original), NLB (Network Load Balancer, Layer 4 TCP/UDP), and ALB (Application Load Balancer, Layer 7 HTTP/HTTPS - not used by OpenShift for API/ingress). OPTIONS: 'Classic' (legacy Elastic Load Balancer, Layer 7 for HTTP/HTTPS and Layer 4 for TCP) - older technology, still works but being phased out by AWS. 'NLB' (Network Load Balancer, Layer 4 TCP/UDP with optional TLS termination) - recommended for most installations, better performance, lower latency, static IP support, more efficient health checks. RECOMMENDATION: Use NLB unless you have a specific compatibility requirement for Classic ELB. NLB is the modern choice with better performance characteristics. WHY NLB IS BETTER: (1) Lower latency (Layer 4 forwarding vs Layer 7 proxy). (2) Can handle millions of requests per second. (3) Static IP addresses (useful for firewall rules, IP whitelisting). (4) Preserves source IP by default (better for audit logs, access control). (5) More cost-effective at scale. (6) Better TLS termination options. WHEN TO USE CLASSIC: (1) Legacy environments with existing Classic ELB dependencies. (2) Very specific Layer 7 features needed (rare for OpenShift). (3) Compatibility with older AWS SDK/tooling that doesn't support NLB. IMPORTANT: This setting affects BOTH the API load balancer (for accessing the Kubernetes API at api.<cluster>.<domain>) and the default ingress controller load balancer (for routing app traffic to *.apps.<cluster>.<domain>). You can change the ingress load balancer type post-install via IngressController resource, but the API load balancer type is set at install time. For most GovCloud installations, NLB is the right choice. Example: Choose 'NLB' for new installations unless you have a specific reason not to."}
                  >
                    <select
          ...
          <section className="card">
            ...
            <div className="card-body">
              <div className="field-grid" style={{ marginTop: 12 }}>
                ...
                <FieldLabelWithInfo
                  label="Region"
                  hint={metaAzureRegion?.description || "Azure Government region where the cluster will be deployed. This determines the physical datacenter location for all cluster resources (VMs, storage, networking). Common Azure Government regions: usgovvirginia (US Gov Virginia - primary region, most services), usgovtexas (US Gov Texas - secondary region), usgovarizona (US Gov Arizona), usdodeast (US DoD East - DoD IL5), usdodcentral (US DoD Central - DoD IL5). IMPORTANT: Not all Azure Government regions support all VM types or OpenShift features - usgovvirginia and usgovtexas are the most complete. Check Azure Government region availability for the VM sizes you need. WHY IT MATTERS: The region affects latency (choose closest to your users), compliance (DoD regions for DoD IL5 workloads), and pricing (some regions cost more). All cluster resources (VMs, managed disks, load balancers, NSGs) must be in the same region - you cannot spread a cluster across regions. For high availability across regions, you must deploy separate clusters per region. DATA RESIDENCY: Azure Government guarantees all data stays within the selected US region - critical for compliance requirements. Example: Enter 'usgovvirginia' (no quotes) for US Gov Virginia region. The exact region name must match Azure's region identifier (lowercase, no spaces). You can find region names in the Azure Government portal under Resource Groups → Create → Region dropdown, or via 'az account list-locations --output table' from Azure CLI."}
                ...
                <FieldLabelWithInfo
                  ...
                  hint={metaAzureResourceGroupName?.description || "Name of the Azure resource group where the installer will create all cluster resources (VMs, disks, NSGs, load balancers, public IPs, availability sets, etc.). This resource group must ALREADY EXIST before installation - the installer will not create it. WHY A RESOURCE GROUP: In Azure, a resource group is a container that holds related resources for an Azure solution. All cluster infrastructure goes into this one resource group for easier management, billing tracking, and cleanup (delete the entire cluster by deleting the resource group). IMPORTANT: Use a dedicated resource group for the OpenShift cluster - do not share it with other applications or infrastructure. The installer needs extensive permissions on this resource group (create VMs, networks, storage, etc.). After installation, the cluster's cloud-provider integration continues using this resource group to create resources like persistent volumes (managed disks) and load balancers for services. NAMING: Choose a descriptive name that clearly identifies this cluster (e.g., 'ocp-prod-cluster-rg' or 'openshift-dev-usgovva'). Resource group names are case-insensitive, can contain letters, numbers, hyphens, underscores, periods, and parentheses, and must not end with a period. CREATE IT FIRST: Before running openshift-install, create this resource group in Azure Government portal or via Azure CLI: 'az group create --name <your-rg-name> --location usgovvirginia'. The resource group's region should match the region you specified above. You can find existing resource groups in Azure portal → Resource groups, or list them via 'az group list --output table'. This is DIFFERENT from baseDomainResourceGroupName (which holds the DNS zone) - you can use the same resource group for both if desired, but separating them is common."}
                ...
                <FieldLabelWithInfo
                  ...
                  hint={metaAzureBaseDomainResourceGroupName?.description || "Name of the Azure resource group that contains the DNS zone for your base domain (the parent domain under which the cluster will be created). This resource group must ALREADY EXIST and must contain a properly configured Azure DNS zone matching your base domain. WHAT IS THIS: When you set a base domain like 'example.com' in Identity & Access, OpenShift needs to create DNS records for the cluster (e.g., api.my-cluster.example.com, *.apps.my-cluster.example.com). In Azure, DNS zones are managed resources that live in a resource group - this field tells the installer which resource group holds the DNS zone for your domain. EXAMPLE: If your base domain is 'example.com' and you have an Azure DNS zone for 'example.com' in resource group 'dns-zones-rg', enter 'dns-zones-rg' here. The installer will then create DNS records in that zone for your cluster. CAN IT BE THE SAME AS CLUSTER RESOURCE GROUP? Yes, you can use the same resource group name as 'Resource group name' above if your DNS zone lives in the same resource group where cluster resources will be created. Alternatively, you might have a centralized 'dns-rg' that holds DNS zones for multiple clusters/applications - that's fine too. CRITICAL REQUIREMENT: The Azure service principal or managed identity used for installation must have 'DNS Zone Contributor' role (or equivalent permissions) on this resource group and the DNS zone inside it. Without DNS permissions, the installer cannot create required DNS records and installation will fail. PREREQUISITES BEFORE INSTALL: (1) Create the resource group if it doesn't exist. (2) Create an Azure DNS zone for your base domain in that resource group. (3) Delegate your domain to Azure's nameservers (get NS records from the Azure DNS zone, add them to your domain registrar). (4) Verify delegation works: 'dig NS example.com' should return Azure nameservers. You can find DNS zones in Azure portal → DNS zones, or list them via 'az network dns zone list --output table'."}
            ...
            <div className="card-body">
              ...
              <div className="field-grid" style={{ marginTop: 8, marginBottom: 16 }}>
                <FieldLabelWithInfo
                  ...
                  hint={metaNutanixEndpoint?.description || "Fully qualified domain name (FQDN) or IP address of your Nutanix Prism Central instance. Prism Central is the centralized management interface for Nutanix clusters that the OpenShift installer uses to provision VMs, configure networking, and manage cluster infrastructure. WHAT IS PRISM CENTRAL: Prism Central (PC) is Nutanix's multi-cluster management platform that sits above one or more Prism Element (PE) clusters. It provides unified management, monitoring, and automation across your Nutanix infrastructure. The installer communicates with Prism Central's REST API to create and configure OpenShift VMs. FQDN VS IP: You can use either a fully qualified domain name (e.g., 'prism.example.com', 'pc.nutanix.internal') or an IPv4 address (e.g., '192.168.1.50'). FQDN is recommended for production - it's more maintainable if the IP changes, and supports TLS certificate validation better. REQUIREMENTS: (1) The endpoint must be reachable from the network where you run openshift-install (for disconnected installs, this means routable within your private network). (2) The Prism Central instance must be running and accessible on the API port (default 9440, see 'Port' field below). (3) For TLS/HTTPS (default), ensure the TLS certificate is valid or you have proper certificate trust configured (Prism Central uses self-signed certs by default - you may need to import the CA cert into your trust store). (4) The credentials you provide below must have admin-level permissions in Prism Central to create VMs, networks, and storage. VERSION COMPATIBILITY: Ensure your Prism Central version is compatible with the OpenShift version you're installing - consult OpenShift documentation for the support matrix. Typically Prism Central 2020.9+ is required for OpenShift 4.9+, and newer OpenShift versions may require newer PC versions. Example: 'prism-central.example.com' or '10.50.100.20'."}
                ...
                <FieldLabelWithInfo
                  ...
                  hint={metaNutanixPort?.description || "TCP port number for the Prism Central REST API. Leave blank to use the default port 9440 (standard Prism Central HTTPS port). WHAT IS THIS: Prism Central's management API listens on a specific TCP port for HTTPS connections. The OpenShift installer connects to this port to execute API calls for VM provisioning, network configuration, and cluster management. Port 9440 is the Nutanix default for Prism Central. WHY YOU MIGHT CHANGE IT: (1) Your organization uses non-standard ports for security policy compliance. (2) Prism Central is behind a reverse proxy or load balancer that uses a different port. (3) Network address translation (NAT) or port forwarding routes traffic to a different external port. (4) Multiple Prism Central instances on the same network with port separation. IMPORTANT: The port must match whatever is actually configured on your Prism Central instance. If you specify the wrong port, the installer will fail to connect with timeout or connection refused errors. The port must allow HTTPS/TLS traffic (Prism Central uses HTTPS by default, not plain HTTP). For most installations, leaving this field blank (using default 9440) is correct. Example: Leave blank for standard setups, or enter '443' if PC is behind a reverse proxy on standard HTTPS port, or '8443' if using a custom port."}
                >
                  <input
                    type="number"
              ...
              <div className="field-grid" style={{ marginBottom: 16 }}>
                <FieldLabelWithInfo
                  label="Username"
                  hint={metaNutanixUsername?.description || "Username for authenticating to Prism Central with administrative privileges. The OpenShift installer uses these credentials to provision VMs, configure networking, and manage cluster infrastructure via the Prism Central API. REQUIRED PERMISSIONS: The user must have admin-level permissions in Prism Central, specifically: (1) Ability to create and manage VMs (including power operations, configuration changes, disk management). (2) Ability to create and configure virtual networks/VLANs. (3) Ability to allocate and manage storage (create volumes, attach disks). (4) Read access to cluster configuration and resources. Typically, the built-in 'admin' user has all required permissions. For production environments, you may want to create a dedicated service account with specific OpenShift-related permissions instead of using the default admin account (consult Nutanix documentation for role-based access control / RBAC setup). AUTHENTICATION METHODS: Prism Central supports local users (managed in Prism Central itself) and directory-integrated users (Active Directory, LDAP). Either can be used as long as the user has the required permissions. The username format depends on your authentication source - local users are typically simple usernames like 'admin' or 'openshift-svc', while directory users might be formatted like 'DOMAIN\\username' or 'user@domain.com'. IMPORTANT: The credentials are stored in plain text in the install-config.yaml file unless you choose to exclude them at export. After installation, you can remove credentials from the config file if needed. Never store install-config.yaml with credentials in version control or shared storage. Example: 'admin' for local admin user, or 'openshift-installer@corp.example.com' for a dedicated directory service account."}
                >
                  <input
                ...
                <FieldLabelWithInfo
                  label="Password"
                  hint={metaNutanixPassword?.description || "Password for the Prism Central username specified above. This credential is used during OpenShift installation to authenticate API calls for provisioning VMs, configuring networks, and managing cluster infrastructure. The installer stores this password in plain text in the install-config.yaml file (unless you choose to exclude credentials at export time), so treat the install-config with appropriate security controls. IMPORTANT SECURITY NOTES: (1) DO NOT allow your browser to save this password - it will be embedded in plain text in the generated config file. Use your browser's password manager ignore features if prompted. (2) After installation completes, you can remove the credentials from install-config.yaml if you no longer need them (though some day-2 operations may require them). (3) Store install-config.yaml securely - never commit it to version control or place it in shared/public storage with credentials included. (4) Consider using a dedicated service account with limited permissions instead of the main admin account for better security and auditability. (5) Rotate credentials periodically and update any stored install-config files accordingly. CREDENTIAL INCLUSION: When you export/generate the install-config, you'll have an option to include or exclude credentials. If excluded, you must provide credentials separately when running openshift-install (via prompts or environment variables). If included, the install-config is self-contained but more sensitive. For production deployments, many organizations use temporary credentials that are rotated or revoked after installation, or use secrets management tools (HashiCorp Vault, AWS Secrets Manager) instead of plain text storage."}
                >
...
12345678-abcd-1234-5678-abcdef123456 or 'Production-VLAN100' or 'OCP-Network'"
                ...
                <FieldLabelWithInfo
                  ...
                  hint={metaNutanixClusterName?.description || "Name of the Nutanix Prism Element (PE) cluster where OpenShift virtual machines will be deployed. Leave blank to let the installer choose automatically (most common). WHAT IS THIS: Nutanix uses a two-tier management model - Prism Central (PC) manages multiple Prism Element clusters. Each PE cluster is a group of Nutanix nodes (hyperconverged servers) that form a storage/compute pool. This field specifies which PE cluster hosts your OpenShift VMs. WHY SPECIFY ONE: Set a cluster name when you have multiple PE clusters managed by your Prism Central and want to pin OpenShift to a specific cluster (e.g., for capacity isolation, SLA tiers, or licensing/billing separation). If left blank, the installer selects a PE cluster automatically based on available resources. IMPORTANT: This field expects the CLUSTER NAME (human-readable string like 'Production-Cluster-01'), NOT the cluster UUID. You can find cluster names in Prism Central → Compute & Storage → Clusters. The cluster must be registered with the Prism Central endpoint you specified above, and must have sufficient resources (CPU, memory, storage) for your control plane and worker node requirements. The cluster must also be running compatible Nutanix AOS/AHV versions (consult OpenShift documentation for version compatibility matrix). Example: 'Production-Cluster-01' or 'NX-3060-Cluster'. Most single-cluster Nutanix deployments leave this blank."}
                >
                  <input
