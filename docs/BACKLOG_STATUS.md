...
| DOC-058 | Rework scenario summary dropdown: live updating + documentation sources | active | p2 | user backlog intake (2026-05-10), `COMPREHENSIVE_MASTER_PLAN.md` Phase 4.1 | `frontend/src/steps/` (scenario summary dropdown), Field Guide generation logic | **Goal:** Scenario summary dropdown tells a **live-updating story** like Field Guide initial summary section. **Requirements:** (1) **Live update values** as user makes selections (proxy enabled/disabled, FIPS on/off, NTP servers configured, dual-stack, mirror registry, etc.). Summary dynamically reflects user's actual configuration. (2) **Documentation sources section:** Replicate accurate documentation sources section from Field Guide with **live real-time links** to docs relevant to user's specific scenario + configurations. Review Field Guide generation logic for these two sections and apply same behavior to scenario summary dropdown. |
