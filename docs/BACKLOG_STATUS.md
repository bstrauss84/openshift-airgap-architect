...
| DOC-037 | Design high-side/low-side local-first operating modes with capability gating | verified_done | p1 | user backlog intake (2026-03-24), governance and security docs | `backend/src/index.js` profile contract + capability-gated routes (`/api/profile/capabilities`, release/docs/operators/aws/update/feedback/oc-mirror), runtime package preload/profile handling, frontend gating in `frontend/src/steps/BlueprintStep.jsx`, `frontend/src/steps/OperatorsStep.jsx`, `frontend/src/steps/ReviewStep.jsx`, `frontend/src/steps/RunOcMirrorStep.jsx` | Keep regression validation for packaged runtime startup (`load-runtime-images.sh` + `start-high-side.sh`), localhost-only bind behavior, bundled payload preload, and connected-only action blocking in CI/manual release checklist. |
...
| DOC-050 | Stabilize Review-step toggles, export-control dedupe, and segmented-flow legacy fallback guard | done_pending_verification | p0 | stabilization pass request (2026-04-11), `README.md`, `docs/CONTRIBUTING.md` | `frontend/src/components/Switch.jsx`, `frontend/src/steps/ReviewStep.jsx`, `frontend/src/App.jsx`, `frontend/src/exportInclusion.js`, `backend/src/exportInclusion.js`, `backend/src/index.js`, `frontend/tests/review-step-stabilization.test.jsx`, `frontend/tests/segmented-flow-flag.test.jsx`, `backend/test/continuation-and-export-behavior.test.js` | Keep targeted frontend/backend regression tests green and complete one final host-level runtime package signoff run (with real image archives) before promoting to `verified_done`. |
...
|---|---|---|---|---|---|
| DEF-001 | Placeholder values non-destructive architecture | done_pending_verification | p1 | `docs/PLACEHOLDER_VALUES_DEFERRED.md`, `backend/src/placeholderEngine.js`, `frontend/src/placeholderEngine.js`, `frontend/src/steps/HostInventoryV2Step.jsx`, `backend/src/fieldGuide/assembler.js` | Hardened utility-row UX for repeated host/network fields, expanded readiness/field-manual wording, and added marker-rendering + execution-blocking tests. Remaining verification gate: browser-level visual pass for helper-icon/spacing integrity on touched steps before promoting to `verified_done`. |
