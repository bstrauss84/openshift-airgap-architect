...
| DOC-057 | Preview panels always display after Blueprint lock-in with incomplete warnings | active | p2 | user backlog intake (2026-05-10) | `frontend/src/steps/AssetsGuideStep.jsx`, preview panel components | **Current behavior:** Preview panels (install-config, agent-config, ImageSet config) only show when files are complete. **Goal:** Always display preview panels after Blueprint lock-in, even when incomplete. **Requirements:** Show panels with clear **incomplete warnings** identifying missing fields/sections before treating files as valid. Visual indicators (warning icon, incomplete badge) + tooltip/banner listing what remains. Users see progress and understand what's blocking completion rather than seeing empty/hidden panels. |
...
| DOC-060 | Include latest mirror-registry binary in downloadable assets | active | p2 | user backlog intake (2026-05-10) | `frontend/src/steps/AssetsGuideStep.jsx`, backend bundle generation | Add option/toggle on Assets & Guide tab to include latest version of **mirror-registry binary** in downloadable assets bundle. |
| DOC-061 | Double width of HTTP/HTTPS/no proxy fields | active | p2 | user backlog intake (2026-05-10) | `frontend/src/steps/TrustProxyStep.jsx`, `frontend/src/styles.css` | **Goal:** Double the width of the 3 proxy fields: HTTP proxy, HTTPS proxy, and no proxy. **Current status:** Fields likely use standard width. **Target:** 2x current width for better readability of long proxy URLs. Cross-reference with DOC-050 credential field width expansion (700px → 875px) for consistent sizing approach. |
| DOC-062 | Clear selections button for Operators tab | active | p2 | user backlog intake (2026-05-10) | `frontend/src/steps/OperatorsStep.jsx` | Add **"Clear selections"** (or similar label) button in the **"Selected operators"** section of Operators tab. **Behavior:** Clear out ALL selected operators and re-add them back to their respective operator catalog lists (Red Hat Operators, Certified Operators, etc.). |
