...
| DOC-064 | Exclude Assets & Guide, Run oc-mirror, and Operations tabs from checkmark/needs review indicators | active | p2 | user backlog intake (2026-05-11) | Navigation/menu indicator logic, `frontend/src/` (step completion and review flag components) | **Current behavior:** Assets & Guide, Run oc-mirror, and Operations tabs can show checkmarks and/or "needs review" labels in the navigation menu. **Desired behavior:** Completely exclude these 3 tabs from the checkmark/needs review indicator system. These are operational/output tabs, not configuration input tabs, so completion indicators don't apply. **Scope:** Modify menu/navigation indicator logic to skip these tabs when calculating completion state and review flags. |
| DOC-065 | Operators tab: Add collapsible top section with header label | active | p2 | user backlog intake (2026-05-11) | `frontend/src/steps/OperatorsStep.jsx` | **Current:** Top section (Operator Catalog Selection) has no header label and is always expanded. **Goal:** (1) Add relevant section header label for top section. (2) Make it collapsible/expandable exactly like "Scan status" section below it. (3) Default state: **collapsed**. **Scope:** Add CollapsibleSection wrapper to top section matching pattern used for Scan Status and ImageSet options sections. |
| DOC-066 | Operators tab: Conditional auto-expansion of Scan Status section | active | p2 | user backlog intake (2026-05-11) | `frontend/src/steps/OperatorsStep.jsx` (Scan Status section state logic) | **Current:** Scan Status section default state (collapsed or expanded) is static. **Goal:** Make Scan Status section **conditionally auto-expand** on tab entry if ANY of the 3 scans (Red Hat, Certified, Community) has status OTHER than "completed successfully". **Trigger statuses:** "waiting to scan", errors, "in progress", any non-success state. **User override:** Manual toggle (collapse/expand) overrides auto-expansion behavior. **Default when all scans successful:** collapsed. **Scope:** Add conditional logic to determine initial collapsed state based on scan statuses. |
| DOC-067 | ImageSet options fields: Replace notes with standardized tooltips | active | p2 | user backlog intake (2026-05-11) | `frontend/src/steps/OperatorsStep.jsx` (ImageSet options section), NTP servers tooltip on Connectivity & Mirroring tab (reference format) | **Fields to enhance:** (1) **"Additional images to mirror"** - remove current note "One image reference per line (e.g. quay.io/org/app:tag). Added to mirror.additionalImages in ImageSetConfiguration." and replace with standardized tooltip using bold yellow header format. (2) **"Archive chunk size"** - remove current note "Split oc-mirror output into archive files of this size. Leave blank for the default (500 GiB). Sets the top-level archiveSize field in ImageSetConfiguration (oc-mirror v2). Archives are named mirror_000001.tar, mirror_000002.tar, etc." and replace with standardized tooltip. **Format reference:** Use NTP servers tooltip on Connectivity & Mirroring tab as the canonical format example (proper bold yellow headers with ** markdown). **Also:** ImageSet options section should default to **expanded** state. |
| DOC-068 | Add validation error tooltips to field alert icons | active | p2 | user feedback (2026-05-11); screenshot showing double alert icons with no hover/click feedback | `frontend/src/` (validation error display components) | **Problem:** Validation error alert icons (⚠️ symbols) show on fields with errors but provide NO feedback on hover or click about what the error is. User sees double alert icon but cannot discover what validation failed. **Requirements:** (1) Add hover tooltip showing validation error message. (2) Consider click handler for longer error messages (popup/modal). (3) Ensure ARIA accessibility for screen readers. **Example:** Machine Network field shows two red alert icons but no indication that error is "CIDR overlap" or "Invalid format" or similar. User must guess or look elsewhere for error message. |
