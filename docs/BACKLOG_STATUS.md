...
| DOC-058 | Rework scenario summary dropdown: live updating + documentation sources | done_pending_verification | p2 | user backlog intake (2026-05-10), `COMPREHENSIVE_MASTER_PLAN.md` Phase 4.1 | Commits 1e04247, d9f4294; `frontend/src/scenarioSummaryHelpers.js`, `frontend/tests/scenario-summary-helpers.test.js`, `ScenarioHeaderPanel.jsx`; 44 passing tests | Implemented with tab confirmation tracking, dynamic config sections, and documentation sources. State path bugs found (reading from wrong state locations). Fix in progress 2026-05-11. |
...
| DOC-069 | Fix scenario summary state path bugs (FIPS, proxy, NTP, certs) | active | p0 | user testing (2026-05-11); Field Guide context.js comparison | `frontend/src/scenarioSummaryHelpers.js` lines 73, 194-211, 167-180 | **Critical bug:** Scenario summary helpers read from WRONG state paths. **Backend (Field Guide) reads from:** `state.globalStrategy.fips`, `state.globalStrategy.proxyEnabled`, `state.globalStrategy.proxies.*`, `state.globalStrategy.mirroring.*`, `state.globalStrategy.ntpServers`, `state.trust.*`. **Frontend helpers incorrectly read from:** `state.credentials.fipsMode` (wrong!), `state.strategy.*` (wrong!), `state.mirroring.*` (wrong!), `state.connectivity.*` (wrong!). **Fix:** Align all state paths with backend Field Guide context builder (`backend/src/fieldGuide/context.js` lines 20-171). **Impact:** FIPS shows "Disabled" when enabled, proxy not showing, NTP not showing, certificate counts not showing. |
