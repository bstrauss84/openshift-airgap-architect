...
| DOC-057 | Preview panels always display after Blueprint lock-in with incomplete warnings | verified_done | p2 | user backlog intake (2026-05-10), Phase 1 completion (2026-05-10) | Commit deb267c, `frontend/src/App.jsx` (previewEnabled logic expanded to all steps after Blueprint lock-in), incomplete warning banner in preview panel when errorFlags[previewStepId] | Preview enabled on all steps (methodology, identity-access, networking, connectivity-mirroring, platform-specifics, trust-proxy, inventory-v2, operators, global, review) after Blueprint confirmation. Shows "⚠️ Incomplete Configuration" warning when validation errors exist. Verified 2026-05-10. **Note:** Full YAML drawer (DOC-034 Phase 3) with syntax highlighting, resizable panels, multi-config layout, download buttons still pending. |
