...
**DOC-104 status: active / partial** — not "NOT STARTED". Implemented evidence: migration tests (v1→v3 state schema migration), catalog tests (catalog loading and mirror identity verification), visibility tests (isCatalogFieldVisible with supportStatus), validation tests (version-gated validation for throughput, confidentialCompute, allowSharedKeyAccess), generation tests (version-gated generation with isVersionGTE guards), **version-manifest.json M02 manifest generation and tests** (`backend/src/exportIntegrity.js`, `backend/test/export-integrity.test.js` 25 tests), **M03 validation-core and archive-buffer adapter** (`validateArchiveManifest` fail-closed parsed-manifest validator + `validateArchiveBuffer` in-memory ZIP parsing/validation adapter with CRC32/deflate/stored support, resource limits, and full safety checks; `backend/test/import-integrity.test.js` 174 tests), **M03 HTTP archive-validation endpoint** (`POST /api/bundle.import` registered before global JSON parser with route-local `express.raw()`, content-type gate, 512 MiB limit, validation-only semantics — no state import, no persistence, no filesystem extraction; deterministic error codes mapped to 400/413/415/422/500 per frozen contract; `backend/test/bundle-import.test.js` 27 tests including non-persistence contract tests — persistence/import product semantics resolved as validation-only by design). Remaining: parameterized version-matrix testing across all version-gated behavior, Field Guide version-correctness tests, deprecation badge rendering tests, versioned copy correctness tests.
