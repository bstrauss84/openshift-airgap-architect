...
export const azureGovPrereqs = {
  ...
  items: [
    ...
    { text: "If using an existing VNet (BYO VNet): configure virtualNetwork, networkResourceGroupName, controlPlaneSubnet, and computeSubnet in install-config.yaml. OpenShift 4.20 supports exactly one node subnet (computeSubnet). If multiple node subnets are present (for example from an imported 4.21 configuration), all values are preserved in state but generation is blocked until extras are removed. The app generates flat controlPlaneSubnet and computeSubnet fields for 4.20." },
