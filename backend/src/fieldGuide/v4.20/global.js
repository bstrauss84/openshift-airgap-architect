...
export const ntpConfig = {
  ...
  items: [
    ...
    { text: "The generated MachineConfig manifests (99-chrony-ntp-master.yaml, 99-chrony-ntp-worker.yaml) embed the NTP servers for cluster nodes. Apply them after install:", cmd: "oc create -f 99-chrony-ntp-master.yaml\noc create -f 99-chrony-ntp-worker.yaml" },
...
export const toolsAndCreds = {
  ...
  items: [
    ...
    { text: "Place the imagesetconfiguration file ({{imageSetConfig}}) in your working directory and verify its syntax:", cmd: "cat {{imageSetConfig}}" },
    { text: "Ensure the install-config.yaml is complete and validated. Run a preflight check:", cmd: "openshift-install create manifests --dir {{installDir}} 2>&1 | head -20\n# If manifests are created without error, install-config.yaml is valid." },
    { text: "Back up install-config.yaml before running any installer commands (the installer consumes it):", cmd: "cp {{installDir}}/install-config.yaml {{installDir}}/install-config.yaml.bak" },
    { text: "If installing on bare metal with agent-based installer, also confirm agent-config.yaml is present in {{installDir}}." },
  ],
};
...
export const day2Basics = {
  ...
  items: [
    ...
    { text: "Apply the NTP MachineConfigs if NTP servers were specified:", cmd: "oc create -f 99-chrony-ntp-master.yaml\noc create -f 99-chrony-ntp-worker.yaml\n# Monitor rollout:\noc get mcp" },
