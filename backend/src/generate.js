/**
 * OpenShift Airgap Architect - Configuration Generator
 *
 * Generates install-config.yaml, agent-config.yaml, and Field Guide documentation
 * from user input. Supports all platforms: Bare Metal (IPI/UPI/Agent), vSphere,
 * AWS GovCloud, Azure Government, Nutanix, IBM Cloud.
 *
 * State keys follow camelCase (blueprint, globalStrategy, hostInventory, platformConfig).
 *
 * @author Bill Strauss
 *
...
const buildInstallConfig = (state) => {
  ...
  if (state.blueprint?.platform === "AWS GovCloud" && ["IPI", "UPI"].includes(state.methodology?.method)) {
    ...
    if (state.methodology?.method === "IPI" && (platformConfig.aws?.controlPlaneInstanceType || platformConfig.aws?.rootVolumeSize || platformConfig.aws?.rootVolumeType || platformConfig.aws?.rootVolumeIops || platformConfig.aws?.rootVolumeKmsKeyArn || awsThroughputEligible)) {
      ...
      if (platformConfig.aws.rootVolumeSize != null || platformConfig.aws.rootVolumeType || platformConfig.aws.rootVolumeIops || platformConfig.aws.rootVolumeKmsKeyArn || awsThroughputEligible) {
    ...
    if (state.methodology?.method === "IPI" && (platformConfig.aws?.workerInstanceType || platformConfig.aws?.rootVolumeSize || platformConfig.aws?.rootVolumeType || platformConfig.aws?.rootVolumeIops || platformConfig.aws?.rootVolumeKmsKeyArn || awsThroughputEligible)) {
  ...
  const header = [
    ...
    "# https://github.com/billstrauss/openshift-airgap-architect",
    "# Developed by Bill Strauss with AI assistance from Claude (Anthropic) and Cursor AI",
    "#",
    "# IMPORTANT: Validate all configuration values against official Red Hat OpenShift",
    "# Container Platform documentation before use in production environments.",
    "",
  ].join("\n");
